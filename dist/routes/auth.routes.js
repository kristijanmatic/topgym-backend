...
authRouter.post('/phone/request', loginLimiter, async (req, res) => {
...
authRouter.post('/phone/verify', loginLimiter, async (req, res) => {
...
authRouter.post('/reception/verify', loginLimiter, async (req, res) => {
    ...
    if (usedAt)
        return res.status(401).json({ error: 'code_used' });
    ...
    const userId = record.user ?? record.userId;
    if (!phone && !userId)
        return res.status(400).json({ error: 'code_not_bound' });
    let user;
    if (phone)
