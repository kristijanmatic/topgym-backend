...
doorRouter.post('/open', doorLimiter, requireAuth, requireRole('admin'), async (req, res) => {
    const body = z
        .object({
    ...
    req.log?.info({ userId: req.auth?.sub, action: body.action, smartlockId: process.env.NUKI_SMARTLOCK_ID }, 'door_open_requested');
